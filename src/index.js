...
const snake = {
     ...
     */
    makeStep() {
        let nextPoint = this.getNextStepHeadPoint()
        for (const i in this.body) {
            let oldPoint = this.body[i];
            this.body[i] = nextPoint;
            nextPoint = oldPoint;
        }
    },
...
const game = {
    ...
    getRandomFreeCoordinates() {
        return {x: this.getRandomInt(this.config.getRowsCount()), y: this.getRandomInt(this.config.getColsCount())}
    },
    ...
    canSetDirection(direction) {
        return ["up", "down"].includes(this.snake.direction) && ["left", "right"].includes(direction) ||
                ["left", "right"].includes(this.snake.direction) && ["up", "down"].includes(direction)
    },
     ...
     */
    isGameWon() {
        const xx = this.snake.bodyLenght > this.config.getRowsCount();
        return false;
    },
    ...
    canMakeStep() {
        ...
        for (let i in this.snake.body) {
            const somePoint = this.snake.body[i];
            if (somePoint.x === nextPoint.x && somePoint.y === nextPoint.y)
                return false;
        }
        return true
    },
